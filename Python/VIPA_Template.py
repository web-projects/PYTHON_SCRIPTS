...
START_CTLS_TAG = [
    ...
    #[(0x9F,0x41), b'\x00\x01' ],                    #
    [(0x9F,0x1A), b'\x08\x40' ],                     #Terminal Country Code
    ...
    for idx0 in buf:
        if len(idx0)== 2 and type(idx0[0]) == str and idx0[0] == 'unparsed':
            if conversion == CONVERT_INT:
                if len(idx0[1]) == 1: 
                  return unpack("!B", idx0[1])[0]
                if len(idx0[2]) == 2: 
                  return unpack("!H", idx0[1])[0]
    ...
    while True:
        ...
        if tlv.tagCount(0x48):   #Card Insertion Detected, proceed with ICC or Magswipe handling
            ...
            if emvStatus == EMV_CARD_INSERTED:
                ...
                if tlvEMV.tagCount(0xE2):
                  appLabels = tlvEMV.getTag(0x50)
                  appAIDs = tlvEMV.getTag((0x9F, 0x06))
                  if len(appLabels):
