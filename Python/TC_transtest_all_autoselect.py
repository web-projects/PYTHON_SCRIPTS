...
def processEMV(tid):
    ...
    start_trans_tag = [
         ##[(0x9F, 0x06)],
         ## TRANSACTION AMOUNT
         [(0x9F, 0x02), b'\x00\x00\x00\x00\x12\x02' ],
         [(0x9A), b'\x04\x01\x01'],
         [(0x9C), b'\x00'],
...
def processTransaction():
    ...
    if (cardState != EMV_CARD_INSERTED):
        if (ctls):
            ...
            start_ctls_tag = [
                ...
                [(0x9F, 0x02), b'\x00\x00\x00\x01\x00\x00' ],
                [(0x9A), b'\x20\x03\x23'],
                [(0x9C), b'\x00'],
            ...
            ##conn.send([0xC0, 0xA0, 0x41, 0x00], start_templ)
    ...
    ''' KSN - SLOT IN USE '''
    sred_tag_val = tlv.getTag((0xFF,0x7F))
    if len(sred_tag_val):
        sred_tlv = hexlify(sred_tag_val[0]).decode('utf-8')
        #log.log('SRED: ', sred_tag_val)
        #log.log('SRED: ', sred_tlv)

        tlv = TLV(['dfdf10', 'dfdf11', 'dfdf12', 'dfdb0f']) # provide ALL possible tag values
        sred_tags = tlv.parse(sred_tlv)
        #log.log('SRED TAGS: ', sred_tags)
        
        log.log("KSN:", sred_tags['dfdf11'].upper())
