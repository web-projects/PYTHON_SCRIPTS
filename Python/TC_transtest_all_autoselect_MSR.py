...
# Main function
def processTransaction(custid, password, action):
    ...
    if tranType == 1:
        #print(">>> ff7f", tlv.tagCount((0xFF,0x7F)))
        TC_TCLink.processEMVTransaction(custid, password, action)
	# Check for swipe
    if tranType == 2:
        #print(">>> ff7f", tlv.tagCount((0xFF,0x7F)))
        TC_TCLink.processMSRTransaction(custid, password, action)
	# Check for CLess
    if tranType == 5:
        #print(">>> ff7f", tlv.tagCount((0xFF,0x7F)))
        TC_TCLink.processEMVTransaction(custid, password, action)
...
if __name__ == '__main__':
    ...
    utility.register_testharness_script(
               partial( processTransaction, args.custid, args.password, args.action ))
