...
def ResetDevice():
  ...
  log.warning('DEVICE RESET COMPLETED --------------------------------------------')
  return buf
...
def htmlEntry():
    ...
    html_tags = [
      ...
      [(0xDF, 0xAA, 0x02), b'TEMPLATE_VAR0'], [(0xDF, 0xAA, 0x03), vipa_version],
    ]
    ...
    # Set display to idle - regardless of tx type
    conn.send([0xD2, 0x01, 0x01, 0x00])
    log.log('*** SET DISPLAY TO IDLE ***')
    status, buf, uns = conn.receive()
