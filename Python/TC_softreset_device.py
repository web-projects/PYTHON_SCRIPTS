...
def processReset():
    ...
    #Send reset device: get device info
    # P1 - 0x01 : hard reset
    #    - 0x02 : VIPA restart
    conn.send([0xD0, 0x00, 0x02, 0x41])
    status, buf, uns = getAnswer()
    tlv = TLVParser(buf)
    model = tlv.getTag((0xDF, 0x0D))
    if len(model): 
        model = str(model[0], 'iso8859-1')
        log.log('Terminal MODEL: ', model)
    else: 
        model = ''
        log.logerr('Invalid MODEL (or cannot determine MODEL)!')
