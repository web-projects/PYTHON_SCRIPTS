...
def GenerateHMAC():
    ...
    # AMEX UAT "TEST CARD 12"
    pan = '374245001751006'
    
    # expected VSS6 HMAC for TC test secrets: C464084095AE8D1F16B57602724955651D45B4B6083E4A5E41C4837081F460A6
    ...
    log.log("Generate HMAC sent for HOSTID-06")
    ...
    if (tlv.tagCount(tag_output_data) == 1):
        ...
        # expected VSS7 HMAC for TC test secrets: 
        ...
        conn.send([0xC4, 0x22, 0x00, 0x00] , c_tag.getTemplate(0xE0))
        log.log("Generate HMAC sent for HOSTID-07")
      ...
      # expected result: 0x04 host not found (requested in P1 host configuration was not found)
      if (tlv.tagCount((0xDF, 0xDF, 0x30)) == 1):
          hmac = tlv.getTag((0xDF, 0xDF, 0x30))[0]
          log.log("HMAC ERROR:", hexlify(hmac).decode('utf-8'))
